import { useEffect, useCallback } from "react";
import { usePomodoroStore } from "../../store/pomodoroStore";

export default function PomodoroTimer() {
  const { session, remainingSeconds, isLoading, start, pause, resume, complete, tick, fetchCurrent } =
    usePomodoroStore();

  useEffect(() => {
    fetchCurrent();
  }, [fetchCurrent]);

  useEffect(() => {
    if (session?.state !== "RUNNING") return;
    const interval = setInterval(tick, 1000);
    return () => clearInterval(interval);
  }, [session?.state, session?.id, tick]);

  const minutes = Math.floor(remainingSeconds / 60);
  const seconds = remainingSeconds % 60;
  const display = `${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;

  if (isLoading) {
    return (
      <div className="flex items-center gap-2 rounded-xl bg-gray-100 px-3 py-1.5">
        <div className="h-3 w-3 animate-spin rounded-full border-2 border-primary border-t-transparent" />
      </div>
    );
  }

  if (!session || session.state === "COMPLETED") {
    return (
      <button
        onClick={() => start()}
        className="flex items-center gap-1.5 rounded-xl bg-primary/10 px-3 py-1.5 font-urbanist text-xs font-medium text-primary transition-colors hover:bg-primary/20"
      >
        <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
          <path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        Focus
      </button>
    );
  }

  return (
    <div className="flex items-center gap-2 rounded-xl bg-gray-100 px-3 py-1.5 font-urbanist text-sm font-mono">
      <span className={`tabular-nums ${remainingSeconds < 60 ? "text-red-500" : "text-gray-700"}`}>
        {display}
      </span>
      <div className="flex gap-0.5">
        {session.state === "RUNNING" ? (
          <button
            onClick={pause}
            className="rounded-lg p-1 text-gray-500 hover:bg-gray-200 hover:text-gray-700"
            title="Pause"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10 9v6m4-6v6" />
            </svg>
          </button>
        ) : (
          <button
            onClick={resume}
            className="rounded-lg p-1 text-gray-500 hover:bg-gray-200 hover:text-gray-700"
            title="Resume"
          >
            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
            </svg>
          </button>
        )}
        <button
          onClick={complete}
          className="rounded-lg p-1 text-gray-500 hover:bg-gray-200 hover:text-green-600"
          title="Complete"
        >
          <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
          </svg>
        </button>
      </div>
    </div>
  );
}
